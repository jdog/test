...
, function(ref) {
	...
	return function buildTest(/* path, func (Constructor, Test, TestWaiter, tools), isIntegration */) {
		...
		var Test = tools.Test = tools.Assert = function Test(testName, resultFunc) {
		...
		var TestWaiter = tools.TestWaiter = tools.AsyncAssert = function TestWaiter(testName, resultFunc /* (series, go, call) */ ) {
			...
			if (!scout.asyncTests.length) {
				...
				if (J.exists("test.codeCoverage")) {
					;(function() {
						...
						console.groupEnd()
						console.groupCollapsed("Code Coverage")
