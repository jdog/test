...
// note to future users, I tried upgrading the tracing code for the latest versions of both libraries
// but certain functionality has been removed
	...
	, function load_codeCoverage (ref) {
		...
		return function CodeCoverage () {
			...
			var dog = {
				...
				, clear : function() {
					for (var x in dog.results) delete dog.results[x]
				}
			...
			var mapPoints = dog.mapPoints = function mapPoints(path, script) {
				...
				var modifiedScript = traceInstrument( "fun." + cleanPath + " = " + script, path )

				// create keys based on the name of the file being tested
				if (!dog.results[path]) {
					dog.results[path] = { totalPoints : modifiedScript.functionList.length, hits : {}, misses : {} }

					// happens only one time
					// this means that each test file only loads misses one time
					// the assumption is that the same file will not change
					// what's cool about this is when each test file runs, it cuts down the misses
					var funcList = [].concat( modifiedScript.functionList )
					, element
					while (funcList.length) {

						element = funcList.shift()

						if (!dog.results[path].misses)
							dog.results[path].misses = {}
			...
			J.test.trace = {
				enterFunction : function(obj) {
					...
					if (obj && obj.path) {
						...
						if (!dog.results[obj.path].hits)
							dog.results[obj.path].hits = { }
